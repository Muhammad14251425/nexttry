...
const Hero3 = () => {
    return (
        <div className='pt-20 max-w-7xl mx-auto px-4 pb-20'>
            <h2 className='text-4xl font-semibold text-center sm:text-left'>Our Fleet in Action</h2>
            ...
            <div className='pt-6 grid gap-4 mt-6 grid-cols-3 sm:grid-cols-6'>
                {data.map((item, index) => (
                    <Div
                        initial={{ y: 100, opacity: 0.2 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }}
                        className="overflow-hidden w-[95%] sm:w-[85%] h-40 rounded-md" key={index}>
                        <Image
                            src={item}
                            alt={item}
